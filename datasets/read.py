...
class DN348DatasetManager:
    ...
    def __init__(self, root: Union[str, Path]):
        ...
        assert osp.exists(self.data_root), f"数据集路径不存在: {self.data_root}"
        assert osp.exists(self.split_dir), f"split 文件夹不存在: {self.split_dir}"
    ...
    def _load_split(self, period: str, split: str, store: List[Tuple[Path, int]]):
        ...
        store: 存储列表
        """
        list_file = osp.join(self.split_dir, f"{split}_list_{period}.txt")
        root_dir = osp.join(self.data_root, period)
        assert osp.exists(list_file), f"找不到列表文件: {list_file}"
        assert osp.exists(root_dir), f"找不到根目录: {root_dir}"
...
root = "/media/xw/linux系统/xjy/program_code/dataset/dn_reid/"
dataset = DN348DatasetManager(root)

print("train_day 样本数:", len(dataset.train_day))
print("第一个样本:", dataset.train_day[0])
# 输出: (PosixPath('/.../dn348/day/00834/000.jpg'), 834)

print("test_night 样本数:", len(dataset.test_night))